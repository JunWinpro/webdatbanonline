...
const queryMiddleWare = {
    ...
    ownedRestaurants: (req, res, next) => {
        try {
            const filterModel = {}
            const acceptQuery = ['sort', 'name', 'minPrice', 'maxPrice', 'rating', 'category', 'isOpening', 'city', 'district', 'page', 'pageSize', 'isActive']
            for (let key of Object.keys(req.query)) {
                if (!acceptQuery.includes(key)) {
                    throw new Error(`Invalid query parameter: ${key}`)
                }
                ...
                if (key === 'isActive') {
                    ...
                    filterModel.isActive = req.query[key] === 'true' ? true : false
                }
                if (key === 'sort') {
                    const [sortKey, sortValue] = req.query[key].split('_')
                    if (Number(sortValue) !== 1 && Number(sortValue) !== -1) {
